		...
		for (int i = 0; i < MAX_V_NUM_GRID; i++) {
			for (int j = 0; j < MAX_H_NUM_GRID; j++) {
				...
				source1.setOnDragDetected(new DragEventHandler(source1)); // when dragging source1, do something,
																			// source1 is now source in DragEventHandler
																			// class
					...
					public void handle(MouseEvent event) {
						...
						if (target.getGraphic() == null) {
							...
							if (!circleShown) {
								...
								int[] coord = { (int) target.getLayoutX(), (int) target.getLayoutY() };
								System.out
										.println("This Tower's range: " + towers.get(getTowerIndex(coord)).getRange());
								switch (target.getId()) {
								case "basicTower": {
								...
								towerInfo.show(invisibleLabel, invisibleLabel.getLayoutX(),
										invisibleLabel.getLayoutY());
							}
						}
					}
				});
